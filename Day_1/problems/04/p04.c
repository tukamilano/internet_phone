...
vect3 * mk_point(vect3 *p, double x, double y, double z)
{
  p->x = x;
  p->y = y;
  p->z = z;
...
{
  vect3 *A = malloc(sizeof(vect3));
  vect3 *B = malloc(sizeof(vect3));
  A = mk_point(A, atof(argv[1]), atof(argv[2]), atof(argv[3]));
  B = mk_point(B, atof(argv[4]), atof(argv[5]), atof(argv[6]));
  double a = angle(A, B);
  printf("%f\n", a);
  return 0;
}
